    ...
    def theme_config():
        with open("theme.txt", "r", encoding="utf-8") as file:
            ...
            if datatheme == "themeblue.txt":
                group2_Button.config(image=photo1_p4)
                manual_button.config(image=photo1H)
                ok_button.config(image=photo1H)
                read_and_Type.config(image=photo1H)
                play_mp3_button.config(image=photo1H)
                loop_buttom.config(image=photo1H)
                reset_buttom.config(image=photo1H)
                add_word_button.config(image=photo1H)
